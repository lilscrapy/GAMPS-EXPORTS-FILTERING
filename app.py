...
CATEGORY_COL = "category"  # Assurez-vous que c'est le bon nom de colonne
...
# === ASYNC GPT CALL (adapté pour Streamlit) ===
async def classify_category(session, semaphore, category, api_key, target_keyword):
    # Note: Pour Streamlit, on pourrait aussi utiliser le client synchrone
    # mais gardons l'asynchrone pour la performance sur de nombreuses catégories.
...
# Le reste du code sera ajouté ici au fur et à mesure.
...
with st.sidebar:
    ...
    # Logique pour récupérer la clé API de manière sécurisée
    api_key = os.getenv("OPENAI_API_KEY") or ""

    # Si non trouvée dans .env, essayer les secrets de Streamlit (pour le déploiement cloud)
    if not api_key:
        try:
            if 'OPENAI_API_KEY' in st.secrets:
                api_key = st.secrets['OPENAI_API_KEY']
        except st.errors.StreamlitAPIException: # Gère le cas où aucun fichier secret n'est trouvé
            api_key = ""

    api_key_input = st.text_input(
        "OpenAI API Key",
        type="password",
        value=api_key, # Pré-remplit avec la clé trouvée
...
if uploaded_file is not None and api_key_input:
    try:
        ...
        if CATEGORY_COL not in df.columns:
            ...
            # --- Section des pré-filtres optionnels ---
            ...
            if st.button("Start Analysis and Classification", type="primary"):
                if not target_keyword:
                    ...
                    # Nettoyer l'état de la session pour une nouvelle analyse
